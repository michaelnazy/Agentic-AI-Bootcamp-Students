...
```text
  ...
  1 baseline  top-10, middle ..........      10        ~468         YES      —
  2 select    top-3 above threshold ....       3        ~251         YES     46%
  3 compress  minus near-duplicates ....       2        ~217         YES     54%
  4 order     same content, last ......       2        ~217         YES     54%
```

**Which lever bought the most, and what did it cost you?**
The **Select** lever bought the most immediate value, cutting token costs by 46% by dropping 7 irrelevant memories. It costs nothing but the risk of missing a poorly-embedded needle if the `min_sim` threshold is set too aggressively. However, the **Order** lever cost absolutely zero computational overhead and effectively shifted the model's logic from a generic babble about datasets into a precise, accurate answer.

---

## Done when

- [x] Baseline row written down, from *your* run.
- [x] `select()` works and the row shows a saving.
- [x] `compress()` drops at least one memory and prints the pair and the cosine.
- [x] The `order` row is filled in, with the two answers compared.
- [x] The policy note below is written.
...
**WRITE** 
My `should_remember()` function uses targeted regular expressions to capture durable preferences (e.g., "I prefer metric") and factual interests (e.g., "I'm researching renewable energy"). It deliberately refuses to store transient conversational noise, pleasantries, or the raw questions asked by the user, ensuring the vector database does not fill up with irrelevant garbage.

**SUMMARISE** 
The short-term memory summarizer is triggered by a hard turn count (`SUMMARIZE_AFTER_TURNS = 6`). When triggered, it folds the older turns into a dense, rolling 80-word summary while keeping the last 4 turns (`KEEP_LAST = 4`) perfectly verbatim to maintain the immediate conversational flow. The summary drops conversational filler and resolved tangents but preserves durable facts and open questions.

**EVICT** 
Semantic preferences remain true indefinitely, but episodic memories go stale and lose relevance. I would detect this by applying an exponential decay score (`sim * 0.5^(age/HALF_LIFE)`) targeting the `type` and `ts` (timestamp) metadata, preventing outdated episodes from surfacing simply because of vector similarity. For strict privacy and context rot prevention, I would implement a hard eviction path that explicitly deletes episodic memories older than 180 days.

**THE LEVER** 
I applied the **Compress** lever, which pushed my total prompt size savings to 54% (dropping the billed tokens down to 217) by calculating the cosine similarity between retrieved embeddings. It successfully dropped a duplicate IRENA fact (cosine 0.786). This lever risked permanently deleting genuinely new, nuanced information if the similarity threshold was set too low (e.g., under 0.55), but at a 0.70 threshold, it efficiently stripped out recurring costs without degrading the context.
---
